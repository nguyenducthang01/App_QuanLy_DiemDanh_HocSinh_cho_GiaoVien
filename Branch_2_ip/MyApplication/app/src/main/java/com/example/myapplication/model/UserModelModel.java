...
//                        System.out.println("dbteacher_id "+dbteacher_id);
            ...
            public void onErrorResponse(VolleyError error) {
                Toast.makeText((Context) loginView, "Loi 1", Toast.LENGTH_SHORT).show();
//                Toast.makeText((Context) loginView, error.toString().trim(), Toast.LENGTH_SHORT).show();
                ...
                if (error.getCause() != null) {
                    errorMsg += " | Cause: " + error.getCause().toString();
                }
                Toast.makeText((Context) loginView, errorMsg, Toast.LENGTH_LONG).show();
                System.out.println("VolleyError"+ errorMsg); // Also log it for debugging
            }
        }) {
            @Nullable
            @Override
